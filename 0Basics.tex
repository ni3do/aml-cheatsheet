...
\subsection*{Gaussian}
\begin{inparaitem}[\color{red}\textbullet]
    \item $p(x) = \frac{1}{\sqrt{(2\pi)^n \mid \Sigma \mid}}\exp (-\frac{1}{2}(x-\mu)^\top \Sigma^{-1} (x-\mu))$
    \item $MX \sim \mathcal{N}(M\mu,M \Sigma M^\top)$ 
    \item $X\mid (Y=a) \sim  \mathcal{N}(\mu_X+\Sigma_{XY}\Sigma_{YY}^{-1}(a-\mu_Y),\Sigma_{XX}-\Sigma_{XY}\Sigma_{YY}^{-1}\Sigma_{YX})$
    \item Subset of vars: $X_A \sim \mathcal{N}(\mu_A, \Sigma_{AA})$
    \item $X+Y = \mathcal{N}(\mu_X+\mu_Y, \Sigma_X + \Sigma_Y$ if independent, else use joint dist. ($[1,1] \cdot [X,Y]^\top$)
\end{inparaitem}
\subsection*{Calculus}
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item $\frac{\partial}{\partial \mathbf{X}}(\|\mathbf{X}\|_F^2) = 2\mathbf{X}$
     \item $\frac{\partial}{\partial \mathbf{x}}\mathbf{A}^{-1} = -\mathbf{A}^{-1} \frac{\partial \mathbf{A}}{\partial \mathbf{x}} \mathbf{A}^{-1}$\\ 
